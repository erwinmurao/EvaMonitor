...
router.post('/', (req, res) => {
  ...
  db.prepare(
    `UPDATE station_mold_assignments SET unassigned_at = datetime('now')
     WHERE station_id = ? AND mold_slot = ? AND unassigned_at IS NULL`
  ).run(station_id, mold_slot);

  const result = db.prepare(
    `INSERT INTO station_mold_assignments (station_id, mold_slot, mold_id, current_size, injection_feeder, recipe_id)
     VALUES (?, ?, ?, ?, ?, ?)`
  ).run(station_id, mold_slot, mold_id || null, current_size || '', injection_feeder || 1, recipe_id || null);

  queueSync('station_mold_assignments', result.lastInsertRowid, 'INSERT');
  res.status(201).json({ id: result.lastInsertRowid, station_id, mold_slot });
});

// Unassign a slot
router.delete('/:station_id/:mold_slot', (req, res) => {
  const db = getDb();
  db.prepare(
    `UPDATE station_mold_assignments SET unassigned_at = datetime('now')
     WHERE station_id = ? AND mold_slot = ? AND unassigned_at IS NULL`
  ).run(req.params.station_id, req.params.mold_slot);
