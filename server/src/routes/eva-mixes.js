...
router.get('/', (req, res) => {
  const db = getDb();
  const { line_id, shift_id, limit } = req.query;
...
router.get('/:id', (req, res) => {
  const db = getDb();
  const batch = db.prepare('SELECT * FROM eva_mix_batches WHERE id = ?').get(req.params.id);
...
router.post('/', (req, res) => {
  ...
  const result = db.prepare(
    `INSERT INTO eva_mix_batches (line_id, batch_code, material_code, material_color, recipe_id,
      small_eva_kg, big_eva_kg, total_kg, expansion_ratio, injection_feeder, station_id)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
  ).run(line_id, batch_code, material_code, material_color, recipe_id || null,
  ...
  if (recipe) {
    const decrementInventory = db.transaction(() => {
      ...
      if (recipe.small_material_id && small_eva_kg > 0) {
        const smallInv = db.prepare(
          `SELECT id, current_kg FROM material_inventory
           WHERE material_id = ? AND location LIKE '%Storage' ORDER BY current_kg DESC LIMIT 1`
        ).get(recipe.small_material_id);
        ...
        if (smallInv) {
          ...
          db.prepare(
            `INSERT INTO material_transactions (material_id, type, kg, reference_id, notes)
             VALUES (?, 'consume', ?, ?, ?)`
      ...
      if (recipe.big_material_id && big_eva_kg > 0) {
        const bigInv = db.prepare(
          `SELECT id, current_kg FROM material_inventory
           WHERE material_id = ? AND location LIKE '%Storage' ORDER BY current_kg DESC LIMIT 1`
        ).get(recipe.big_material_id);
  ...
  res.status(201).json({ id: result.lastInsertRowid, batch_code, total_kg });
});
