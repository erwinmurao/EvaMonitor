...
router.post('/', (req, res) => {
  ...
  const db = getDb();
  const result = db.prepare(`INSERT INTO defect_logs (cycle_id, station_id, mold_slot, defect_type_id, quantity, notes)
   VALUES (?, ?, ?, ?, ?, ?)`).run(cycle_id || null, station_id, mold_slot || null, defect_type_id, quantity || 1, notes || '');
...
router.get('/types', (req, res) => {
  const db = getDb();
  const rows = db.prepare('SELECT * FROM defect_types ORDER BY name').all();
  res.json(rows);
});
