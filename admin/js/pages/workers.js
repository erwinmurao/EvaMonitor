...
export function render() {
  return `
    <div class="page-header"><h1>Workers</h1><button class="btn btn-primary" id="btn-add-worker">+ Add Worker</button></div>
    <div class="card"><table class="data-table" id="workers-table">
      <thead><tr><th>ID</th><th>Name</th><th>PIN</th><th>Status</th><th>Actions</th></tr></thead><tbody></tbody></table></div>
...
export async function init() {
  ...
  tbody.innerHTML = workers.map(w => `<tr>
    <td>${w.id}</td><td><strong>${w.name}</strong></td><td>${w.pin}</td>
  ...
  tbody.querySelectorAll('[data-deactivate]').forEach(btn => {
    btn.onclick = async () => {
      if (confirm('Deactivate this worker?')) {
        await api.del(`/workers/${btn.dataset.deactivate}`);
        init();
      }
    };
  });
...
function showWorkerForm(worker) {
  ...
  modal.innerHTML = `
    <div class="modal-backdrop"><div class="modal">
      ...
      <div class="form-group"><label>Name</label><input id="wf-name" value="${worker?.name || ''}"></div>
      <div class="form-group"><label>PIN</label><input id="wf-pin" value="${worker?.pin || ''}" maxlength="4" placeholder="4-digit PIN"></div>
  ...
  document.getElementById('wf-save').onclick = async () => {
    ...
    if (isEdit) await api.put(`/workers/${worker.id}`, data);
