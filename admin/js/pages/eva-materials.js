...
export function render() {
  return `
    <div class="page-header">
      <div>
        <h1>EVA Materials</h1>
        <p>Manage EVA material types, costs, and sack sizes</p>
      </div>
    ...
    <div class="card">
      <div class="card-header">
        <div>
          <h3 class="card-title">Material Catalog</h3>
          <p class="card-subtitle">All EVA material variants with pricing</p>
        </div>
      </div>
...
export async function init() {
  ...
  tbody.innerHTML = materials.length > 0 ? materials.map(m => `
    <tr>
      <td><strong>${m.code}</strong></td>
      <td>${m.color}</td>
      <td>${m.size_type}</td>
      <td>${m.sack_size_kg} kg</td>
      <td>${formatCurrency(m.cost_per_kg)}</td>
  ...
  `).join('') : '<tr><td colspan="7" style="text-align: center; padding: 40px; color: var(--text-secondary);">No materials found</td></tr>';
...
function showMaterialForm(material) {
  ...
  modal.innerHTML = `
    <div class="modal-backdrop"><div class="modal">
      ...
      <div class="form-group"><label>Code</label><input class="form-control" id="matf-code" value="${material?.code || ''}"></div>
      <div class="form-group"><label>Color</label><input class="form-control" id="matf-color" value="${material?.color || ''}"></div>
      <div class="form-group"><label>Size Type</label><select class="form-control" id="matf-size"><option value="Small" ${material?.size_type === 'Small' ? 'selected' : ''}>Small</option><option value="Big" ${material?.size_type === 'Big' ? 'selected' : ''}>Big</option></select></div>
      <div class="form-group"><label>Sack Size (kg)</label><input type="number" step="0.5" class="form-control" id="matf-sack" value="${material?.sack_size_kg || 25}"></div>
      <div class="form-group"><label>Cost per kg</label><input type="number" step="0.01" class="form-control" id="matf-cost" value="${material?.cost_per_kg || 0}"></div>
  ...
  document.getElementById('matf-save').onclick = async () => {
    ...
    if (isEdit) await api.put(`/eva-materials?id=${material.id}`, data);
...
function showQuickPairForm() {
  ...
  modal.innerHTML = `
    <div class="modal-backdrop"><div class="modal">
      ...
      <p style="color: var(--text-secondary); margin-bottom: 16px;">Creates both Small + Big variants at once</p>
      <div class="form-group"><label>Code (e.g. H8003)</label><input class="form-control" id="qp-code"></div>
      <div class="form-group"><label>Color (e.g. Black)</label><input class="form-control" id="qp-color"></div>
      <div class="form-group"><label>Small Sack Size (kg)</label><input type="number" class="form-control" id="qp-small-sack" value="25"></div>
      <div class="form-group"><label>Big Sack Size (kg)</label><input type="number" class="form-control" id="qp-big-sack" value="50"></div>
      <div class="form-group"><label>Small Cost/kg</label><input type="number" step="0.01" class="form-control" id="qp-small-cost" value="3.50"></div>
      <div class="form-group"><label>Big Cost/kg</label><input type="number" step="0.01" class="form-control" id="qp-big-cost" value="4.00"></div>
